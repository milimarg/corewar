...
static void ft_xor_5(t_env *e, t_cursor *cursor, t_var v)
{
    if (ZMASK(v.acb) == IDR) {
        ...
        fill_r3_xor(cursor, v, cursor->reg[v.r1 - 1], get_dir(e, cursor, 3, 4));
...
static void ft_xor_4(t_env *e, t_cursor *cursor, t_var v)
{
    if (ZMASK(v.acb) == DIR) {
        ...
        fill_r3_xor(cursor, v, get_ind(e, cursor, 2), get_ind(e, cursor, 4));
...
static void ft_xor_3(t_env *e, t_cursor *cursor, t_var v)
{
    if (ZMASK(v.acb) == DRR) {
        ...
        fill_r3_xor(cursor, v, get_dir(e, cursor, 2, 4),
...
static void ft_xor_2(t_env *e, t_cursor *cursor, t_var v)
{
    if (ZMASK(v.acb) == RIR) {
        ...
        v.r3 = e->a[MODA(cursor->index + 5)].hex;
        fill_r3_xor(cursor, v, cursor->reg[v.r1 - 1], get_ind(e, cursor, 2));
        ft_update_cursor(e, cursor, 6);
    } else
        ft_xor_3(e, cursor, v);
    if (v.carry)
...
void ft_xor(t_env *e, t_cursor *cursor)
{
    ...
    if (ZMASK(v.acb) == RRR) {
        ...
        fill_r3_xor(cursor, v, cursor->reg[v.r1 - 1], cursor->reg[v.r2 - 1]);
        ft_update_cursor(e, cursor, 5);
    } else
        ft_xor_2(e, cursor, v);
    if (v.carry)
        cursor->carry = (!cursor->reg[v.r3 - 1]);
}
